...
1. Install or make the `cosheaf` CLI available. For release-aligned local
   testing, install the framework package from the `v0.2.4` tag:
...
With `tcs-cosheaf` `v0.2.4`, context packs can display formal-link metadata
...
The provider smoke installs the framework from the `v0.2.4` tag by default.
