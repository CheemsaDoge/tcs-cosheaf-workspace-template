...
1. Install or make the `cosheaf` CLI available. For release-aligned local
   testing, install the framework package from the `v1.0.0` tag:
...
With `tcs-cosheaf` `v1.0.0`, context packs can display formal-link metadata
...
`COSHEAF_FRAMEWORK_REF`, defaulting to the published `v1.0.0` tag. It records a
...
For the published `v1.0.0` reviewable-workflow handoff path, run:
...
That demo uses the active local framework checkout when `../tcs-cosheaf`
exists, otherwise it installs the published `v1.0.0` tag. It exercises the
...
This demo installs or uses the published `v1.0.0` framework by default. It
...
otherwise it installs the published `v1.0.0` tag. Set
...
For the published `v1.0.0` bounded research-loop workflow, run:
...
The provider smoke installs the framework from the `v1.0.0` tag by default.
