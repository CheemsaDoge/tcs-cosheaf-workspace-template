...
The demo installs `tcs-cosheaf` from the `v1.0.0` tag, inspects the workspace,
...
The demo installs `tcs-cosheaf` from the `v1.0.0` tag by default. Override the
...
This demo exercises the v1.0.0 CLI-first run loop: it starts a research run,
...
The demo installs `tcs-cosheaf` from the `v1.0.0` tag by default. Override
...
Run the `v1.0.0` strategy-planner workflow:
...
the pinned `v1.0.0` tag. You can also set the local checkout explicitly:
...
Run the published `v1.0.0` bounded research-loop workflow:
...
The demo installs the framework from the published `v1.0.0` tag by default.
...
Run the `v1.0.0` operator-session and review-handoff preview workflow:
...
`COSHEAF_FRAMEWORK_REF`, defaulting to the published `v1.0.0` tag.
...
installs the published `v1.0.0` tag. You can override the source with
...
The smoke installs `tcs-cosheaf` from the `v1.0.0` tag by default. Real hosted
...
`make reviewable-workflow-demo` runs the published `v1.0.0` reviewable-workflow
...
a sibling framework checkout, the published `v1.0.0` tag, or explicit
